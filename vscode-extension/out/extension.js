...
function activate(context) {
    ...
    const recordCommand = vscode.commands.registerCommand('reccall.record', async () => {
        ...
        if (shortcuts[shortcut]) {
            const overwrite = await vscode.window.showWarningMessage(`Shortcut '${shortcut}' already exists. Overwrite?`, 'Yes', 'No');
            if (overwrite !== 'Yes') {
                return;
            }
        }
    ...
    const callCommand = vscode.commands.registerCommand('reccall.call', async () => {
        ...
        if (editor && autoInsert) {
            const insert = await vscode.window.showInformationMessage(`Shortcut '${selectedShortcut}' loaded. Insert into current editor?`, 'Yes', 'No');
            if (insert === 'Yes') {
    ...
    const deleteCommand = vscode.commands.registerCommand('reccall.delete', async () => {
        ...
        const confirm = await vscode.window.showWarningMessage(`Are you sure you want to delete shortcut '${selectedShortcut}'?`, 'Yes', 'No');
        if (confirm === 'Yes') {
    ...
    const reloadStarterPackCommand = vscode.commands.registerCommand('reccall.reloadStarterPack', async () => {
        ...
        if (count > 0) {
            const confirm = await vscode.window.showWarningMessage(`This will overwrite ALL ${count} existing shortcuts with starter pack recipes. Continue?`, 'Yes', 'No');
            if (confirm !== 'Yes') {
                return;
            }
        }
        ...
        vscode.window.showInformationMessage(`✅ Starter pack loaded successfully! ${Object.keys(starterPack).length} recipes loaded.`);
    });
    // Register all commands
    context.subscriptions.push(recordCommand, callCommand, listCommand, searchCommand, updateCommand, deleteCommand, reloadStarterPackCommand);
}
